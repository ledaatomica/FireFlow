...
- **Cost and scale** — the runner fleet defaults to spot capacity, packs jobs, and scales toward zero when the queue is empty instead of holding a fixed pool sized for the busiest hour.
- **Security** — commits are signature verified at intake, dependencies are scanned per diff, and every build produces a signed artifact with a software bill of materials and a provenance attestation, rechecked again at deploy time.
- **Quality and risk** — test selection, merge ordering, and rollout pacing are all driven by a risk score, so a single line documentation change and a change to a core payment path are not held to the same gate.
...
Open `FireFlow.html` directly in a browser — it's a single self contained file with no build step and no external dependencies. It respects the operating system's light/dark theme preference and reduced motion setting.
